...
문서 양식은 고정(A3 세로, 1·2·3부 표 구조)이지만, 학교마다 다를 수 있는 고정 문구는 최상위 `서식` 키로 코드 수정 없이 바꿀 수 있다. 교사가 학교 규정과 다르다고 하면 이 키를 사용한다.
...
```json
{
  "서식": {
    "방침": ["가. …", "나. …"],
    "유의사항": ["가. …", "나. …"],
    "성취율표": [["90%이상 ~ 100%", "A"], ["80%이상 ~ 90%미만", "B"]]
  },
  "meta": { "...": "..." }
}
```

생략하면 기본 문구(대구 지역 중학교 기준)가 들어간다.
